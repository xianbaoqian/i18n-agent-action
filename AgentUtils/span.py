...
class Span:
    ...
    def display(self, level: int = 0) -> str:
        ...
        """
        indent = "  " * level
        create_time_str = datetime.fromtimestamp(self.create_time / 1e9).strftime('%Y-%m-%d %H:%M:%S.%f')
        
        if self.end_time:
            end_time_str = datetime.fromtimestamp(self.end_time / 1e9).strftime('%Y-%m-%d %H:%M:%S.%f')
        ...
        result += f"{indent}  Create: {create_time_str}, End: {end_time_str}{duration}\n"
    ...
    def from_dict(cls, data: Dict[str, Any], span_mgr: 'Span_Mgr') -> 'Span':
        ...
        # 重建父子关系
        span.parent = span_mgr.get_span_by_hash(data["parent_hash"]) if data["parent_hash"] else None
        ...
        with self._lock:
            ...
            if stored_data:
                try:
                    ...
                    # 重建根Span列表和所有Span映射
                    self.root_spans = [span for span in temp_spans.values() if span.parent is None]
