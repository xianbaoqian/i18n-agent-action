...
class ExpiringDictStorage:
    ...
    def _load_data(self):
        ...
        with self.file_lock:
            ...
            try:
                ...
                if current_time - data['_metadata']['last_clean'] > self.expiry_days * 86400:
                    data['data'] = {k: v for k, v in data['data'].items() 
    ...
    def clean_expired(self):
        ...
        with self.lock:
            current_time = time.time()
            self.data['data'] = {k: v for k, v in self.data['data'].items() 
                               if current_time - v['timestamp'] <= self.expiry_days * 86400}
