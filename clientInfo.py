...
class clientInfo:
    ...
    def get_legal_info(self):
        return "Disclaimers: This content is powered by i18n-agent-action with LLM service " + self._base_url + " with model " + self._model + ", for some reason, (for example, we are not native speaker) we use LLM to provide this translate for you. If you find any corrections, please file an issue or raise a PR back to github, and switch back to default language."
